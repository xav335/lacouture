...
class Reference_image extends StorageManager {
	...
	public function getListe( $tab=array(), $debug=false ){
		$this->dbConnect();
		$requete = "SELECT * FROM `reference_image`" ;
	...
	public function getImageDefaut( $num_reference=0, $section='normale', $debug=false ) {
		//if ( $debug ) echo "num_image : " . $num_image . "<br>";
		
		$this->dbConnect();
		$requete = "SELECT * FROM `reference_image` 
			WHERE num_reference = " . intval( $num_reference ) . "
			AND defaut = 'oui'";
		if ( $debug ) echo $requete . "<br>";
		$result = mysqli_query( $this->mysqli, $requete );
		...
		if ( 1 == 1 ) {
			$image_defaut = ( $row[ "fichier" ] != '' && file_exists( $_SERVER[ "DOCUMENT_ROOT" ] . "/photos/reference/" . $section . "/" . $row[ "fichier" ] ) ) 
				? "/photos/reference/" . $section . "/" . $row[ "fichier" ]
				: "http://www.placehold.it/1200x800/EFEFEF/171717&text=:(";
		}
		// ------------------------------------------- //
		return $image_defaut;
	}
	...
	public function ajouter( $value, $debug=false ) {
		...
		try {
			...
			$sql = "INSERT INTO `reference_image`
				( `num_reference`, `fichier`, `defaut` )
				VALUES (
				". intval( $value[ "num_reference" ] ) .",
	...
	public function supprimer( $num_image=0, $debug=false ) {
		...
		if ( 1 == 1 ) {
			...
			$fichier_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . "/photos/reference/vignette" . $data[ 0 ][ "fichier" ];
			if ( file_exists( $fichier_a_supprimer ) ) {
				if ( $debug ) echo "On supprime " . $fichier_a_supprimer . "<br>\n";
				if ( !$debug ) unlink( $fichier_a_supprimer );
			}
			
			$fichier_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . "/photos/reference/accueil" . $data[ 0 ][ "fichier" ];
			if ( file_exists( $fichier_a_supprimer ) ) {
				if ( $debug ) echo "On supprime " . $fichier_a_supprimer . "<br>\n";
				if ( !$debug ) unlink( $fichier_a_supprimer );
			}
			
			$fichier_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . "/photos/reference/normale" . $data[ 0 ][ "fichier" ];
			if ( file_exists( $fichier_a_supprimer ) ) {
				if ( $debug ) echo "On supprime " . $fichier_a_supprimer . "<br>\n";
				if ( !$debug ) unlink( $fichier_a_supprimer );
			}
		...
		$this->begin();
		try {
			$sql = "DELETE FROM `reference_image` WHERE `num_image` = " . intval( $num_image );
			if ( $debug ) echo $sql . "<br>";
			else {
				$result = mysqli_query( $this->mysqli, $sql );
	...
	public function setChamp( $champ, $valeur=0, $num_image=0, $debug=false ) {
		...
		$this->begin();
		try {
			$requete = "UPDATE `reference_image` SET";
