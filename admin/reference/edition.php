<?
	...
	if ( !empty( $_GET ) ) {
		$action = 'modif';
		$result = $reference->load( $_GET[ "id" ] );

		if ( !empty( $result ) ) {
			$titre_page = 		'Titre "'. $result[ 0 ][ "titre" ] . '"';
			$id =				$_GET[ "id" ];
			$titre = 			$result[ 0 ][ "titre" ];
			$titre_accueil = 	$result[ 0 ][ "titre_accueil" ];
			...
			if ( 1 == 1 ) {
				unset( $recherche );
				$recherche[ "num_reference" ] = $_GET[ "id" ];
	...
	// ---- Ajout d'une rubrique -------------------- //
		...
		$action = 'add';
		$titre_page = 		'Nouvelle réalisation';
		$id	=				null;
		$titre = 			null;
		$titre_accueil = 	null;
		$description =		null;
			...
			<div class="row">
				...
				<div class="col-xs-12 col-sm-12 col-md-12">
					...
					<form name="formulaire" id="formulaire" class="form-horizontal" method="POST" action="traitement.php">
						...
						<div class="form-group" >
							<label class="col-sm-2" for="titre">Titre Accueil :</label>
							<input type="text" class="col-sm-10" name="titre_accueil" required value="<?=$titre_accueil?>">
						</div>
						...
						<div id="div_liste_image">
							...
							if ( !empty( $liste_image ) ) {
								...
								foreach( $liste_image as $_image ) {
				            		...
				            		echo "	<img src='/photos/reference/accueil" . $_image[ "fichier" ] . "' width='230' style='max-width:230px;'></a><br>\n";
